...
const tonePointSchema = {
    ...
    properties: {
        classification: {
            type: Type.STRING,
            description: "The classification of the tone. Must be one of: 'Positive', 'Negative', 'Neutral'.",
        },
        finding: {
            type: Type.STRING,
        },
...
export const analyzeNewsArticle = async (articleUrl: string, language: string): Promise<AnalysisResult> => {
  ...
  try {
    ...
    // Basic validation
    if (parsedJson.articleTitle && parsedJson.neutralSummary && parsedJson.factOnlySummary && parsedJson.eli10Summary && parsedJson.biasAnalysis?.tone?.classification && parsedJson.biasAnalysis?.politicalLeaning?.finding) {
    ...
    if (error instanceof Error && error.message.includes('JSON')) {
        throw new Error("Failed to parse the analysis from the AI model. The format was invalid.");
    }
...
export const fetchLatestNews = async (category: string): Promise<NewsHeadline[]> => {
  const prompt = `
    ...
    [
      {
        "title": "Example Headline About a Political Event",
  ...
  try {
    ...
    if (Array.isArray(parsedJson) && parsedJson.every(item => item.title && item.source && item.url)) {
        ...
        console.warn("Parsed JSON is not in the expected format:", parsedJson);
        return [];
    }

  } catch (error) {
    console.error("Error fetching latest political news:", error);
